...
> Under the hood, `tokenOptions` is passed as-is to [the `getIdToken` method](https://auth0.github.io/one-client-js/classes/earthoOneClient.html#gettokensilently) on the underlying SDK, so all the same options apply here.
...
ngOnInit() {
  this.authService.error$.pipe(
    ...
    mergeMap(() => this.authService.connectWithRedirect())
  ).subscribe();
}
```
...
To ensure the SDK does not process the callback for any provider other than Eartho, configure the AuthModule by setting the `skipRedirectCallback` property as follows:
...
Accept a user invitation through the SDK by creating a route within your application that can handle the user invitation URL, and log the user in by passing the `organization` and `invitation` parameters from this URL. You can either use `connectWithRedirect` or `connectWithPopup` as needed.
...
The express server exposes a single endpoint at `http://localhost:3001/api/external` that needs to be called with an `Authorization` header containing a token 
...
- [Eartho's general contribution guidelines](https://github.com/auth0/open-source-template/blob/master/GENERAL-CONTRIBUTING.md)
- [Eartho's code of conduct guidelines](https://github.com/auth0/open-source-template/blob/master/CODE-OF-CONDUCT.md)

## Support + Feedback

For support or to provide feedback, please [raise an issue on our issue tracker](https://github.com/eartho/one-client-angular/issues).
...
[Why Eartho?](https://auth0.com/why-auth0)

## License

This project is licensed under the MIT license. See the [LICENSE](https://github.com/eartho/one-client-angular/blob/master/LICENSE) file for more info.
# one-client-angular
